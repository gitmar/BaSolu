...
namespace GxPilo.Components.Ictvars
{
    public partial class IctPosts : CompUICrudBase
    {
        public IctPosts(IPendingChangesGuard guard) : base(guard)
        {
        }
        ...
        private void curPlanPlan(int plnId)
        {
            ...
            {
                //int gtib = 7;
                curPlan = orgPlns?.FirstOrDefault(pln => pln.Ptyp == 4);
                InitializeMyDaHies();
                Console.WriteLine($"nb.Rubhies : {curPlan?.Rubhies}");
                //LsGties = LsFixes.Where(ut => ut.Gvars == 36 && ut.Itb == gtib && ut.Elea != 0 && ut.Elea != 9).ToList();
            }
            else
            {
