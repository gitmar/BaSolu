...
{
    public sealed class Old44PendingChangesGuard : IAsyncDisposable
    {
        ...
        public int GetPendingChangesCount()
        {
            //Console.WriteLine("🔍 GetPendingChangesCount ENTRY");

            int ctxCount = 0;
            if (_ctx?.Context?.Entities != null)
            {
                //Console.WriteLine("🔍 _ctx.Context.Entities check");
                ctxCount = _ctx.Context.Entities.Count(e => IsPendingState(e.State));
                //Console.WriteLine($"🔍 ctxCount: {ctxCount}");
            }
            else
            {
                //Console.WriteLine("🔍 No _ctx.Context");
            }

            //Console.WriteLine($"🔍 _childPendingCount: {_childPendingCount}");
            int total = ctxCount + _childPendingCount;

            //Console.WriteLine($"🔍 GetPendingChangesCount EXIT: {total}");
            return total;
        }
        ...
        public void RegisterUnconfirmedPredicate<T>(Expression<Func<T, bool>> predicateExpr) where T : class
        {
            //Console.WriteLine("prob1");
            var compiled = predicateExpr.Compile();  // Safe compile
            ...
            _unconfirmedPredicates[typeof(T)] = obj =>
            {
                ...
                if (obj is T entity)
                {
                    //Console.WriteLine("prob2.5");
                    return compiled(entity);  // No capture!
                }
                //Console.WriteLine("prob3");
                return false;
            };
            //Console.WriteLine($"✅ {typeof(T).Name} registered");
        }
