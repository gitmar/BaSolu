...
namespace GxAdm.Services
{
    ...
    public class MyODataContext : IMyODataContext
    {
        ...
        private readonly HttpClient _httpClient;  // 🔥 For FlushAsync parallel saves

        public MyODataContext(Uri serviceRoot, string otoken, IHttpClientFactory httpClientFactory = null)
        {
            ...
            // Configure OData for LOADING/TRACKING ONLY
            _aocontext.Format.UseJson();
            ConfigureRequestPipeline(otoken);

            // HttpClient for SAVING (PendingChangesGuard)
            _httpClient = httpClientFactory?.CreateClient("ODataClient")
                         ?? new HttpClient { BaseAddress = serviceRoot };
        }
        ...
        public async Task<T> GetByKeyAsync<T>(int key, string entitySetName, string expand = null) where T : class
        {
            ...
            try
            {
                ...
                return enumerator.MoveNext() ? enumerator.Current as T : null;
            }
            catch (Exception ex)
            {
